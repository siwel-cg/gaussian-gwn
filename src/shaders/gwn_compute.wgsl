...
struct GWNUniforms {
    ...
    _pad0: u32,
    _pad1: u32,
    center: vec4<f32>,  // xyz = centroid of splat cloud
};
...
fn compute_gwn(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < gwn_uniforms.num_gaussians; i++) {
        ...
        let p   = vec3<f32>(ab.x, ab.y, cd.x);

        let Sigma = build_covariance(g);
        let ev    = sym3_eigenvalues(Sigma);
        var normal = sym3_min_eigenvector(Sigma, ev.x);

        // Flip normal to point away from cloud centroid.
        // Eigenvectors have no intrinsic sign — this makes orientation consistent.
        if (dot(normal, p - gwn_uniforms.center.xyz) < 0.0) { normal = -normal; }
